...
def create_course(create_course_data):
    ...
    try:
        ...
        s3.put_object(Bucket=Config.BUCKET_NAME, Key=str(course.id) + "/")
...
def delete_course(course_delete_data):
    ...
    if course:
        ...
        s3 = Config.SESSION.client('s3')
        s3.delete_object(Bucket=Config.BUCKET_NAME, Key=str(course.id) + "/")
...
def upload_course_document(course_document_data):
    ...
    try:
        ...
        db.session.commit()
        s3.upload_fileobj(file, Config.BUCKET_NAME, str(document.course_id) + "/" + file.filename,
